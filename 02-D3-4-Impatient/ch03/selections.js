...
function makeKeys() {
  ...
  const scX = d3.scaleLinear().domain([0, 6]).range([50, 300]),
...
function makeSort() {
  const data = ['Jane', 'Anne', 'Mary']

  const ul = d3.select('#sort')
  ul.selectAll('li')
    .data(data)
    .enter()
    .append('li') //<1>
...
function makeUpdate() {
  ...
  const scX = d3.scaleLinear().domain([1, 3]).range([100, 200]), //<2>
    scY = d3.scaleLinear().domain([1, 3]).range([50, 100])

  const svg = d3.select('#update') //<3>

  svg.on('click', function () {
    //<4>
    ;[ds1, ds2] = [ds2, ds1] //<5>

    const cs = svg.selectAll('circle').data(ds1, (d) => d[2]) //<6>

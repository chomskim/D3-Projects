...
function makeBins() {
  const gauss = d3.randomNormal(75, 40)
  const uni = d3.randomUniform(2, 10)
  const data = []
  for (; data.length < 50; ) {
    const g = gauss()
    if (g > 0 && g < 150) {
  ...
  const sc = d3.scaleQuantile().domain(data).range([1, 2, 3])
  ...
  d3.select('#quantile')
    ...
    .attr('cy', () => 115 + uni())
    .attr('cx', (d) => d)
  d3.select('#bd1').attr('x1', sc.invertExtent(1)[1]).attr('x2', sc.invertExtent(1)[1])
  d3.select('#bd2').attr('x1', sc.invertExtent(2)[1]).attr('x2', sc.invertExtent(2)[1])
...
function makeText() {
  ...
  const axMkr = d3.axisLeft(sc).tickValues([0.5])
...
function makeTicks() {
  const sc = d3.scaleLinear().domain([0, 10]).range([0, 200]) //<1>
...
function makeSemilog() {
  d3.text('cost.csv').then((res) => {
    const data = d3.csvParseRows(res, (d) => [+d[0], +d[1]]) //<1>
    ...
    function draw(sel, scX, scY, width, height) {
      ...
      const ds = data.map((d) => [scX(d[0]), scY(d[1])]) //<4>
      sel.append('path').classed('curve', true).attr('d', d3.line()(ds)).attr('fill', 'none')
...
function makeTimeseries() {
  d3.text('load.csv').then((res) => {
    // prepare data
    const parse = d3.utcParse('%H:%M:%S') //<1>
    const format = d3.utcFormat('%H:%M')

    let data = d3.csvParse(res, function (d) {
      //<2>
    ...
    const scT = d3
      ...
      .nice()
      .range([50, 550])
    const scY = d3.scaleLinear().domain([0, 100]).range([250, 50])
    const scC = d3
      .scaleThreshold() //<4>
      .domain([35, 75])
    ...
    // draw data and axes
    const svg = d3.select('#timeseries').attr('cursor', 'crosshair')
    ...
    svg
      .append('g')
      .attr('transform', 'translate(50,0)') //<7>
    ...
    svg.on('mousemove', function () {
      ...
      txt.text(
        format(scT.invert(pt[0])) +
          ' | ' + //<9>
          d3.format('>2d')(scY.invert(pt[1]))
      ) //<10>
    })
  })
}

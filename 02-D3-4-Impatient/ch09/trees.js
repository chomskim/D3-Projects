...
function makeTree() {
  d3.json('filesys.json').then(function (json) {
    const nodes = d3.hierarchy(json, (d) => d.kids) //<1>
...
function makeTree2() {
  d3.json('filesys.json').then(function (json) {
    const nodes = d3.hierarchy(json, (d) => d.kids).sort((a, b) => b.height - a.height)
    d3.cluster().size([250, 225])(nodes)

    const g = d3.select('#tree2').append('g').attr('transform', 'translate(25, 25)')
...
function makeRadial() {
  d3.json('filesys.json').then(function (json) {
    const nodes = d3.cluster().size([2 * Math.PI, 125])(
      //<1>
      d3.hierarchy(json, (d) => d.kids).sort((a, b) => b.height - a.height)
    )

    const g = d3.select('#radial').append('g').attr('transform', 'translate(150, 150)') //<2>

    const h = function (r, phi) {
      return r * Math.sin(phi)
    } //<3>
    const v = function (r, phi) {
...
function makeTreemap() {
  d3.json('filesys.json').then(function (json) {
    const sc = d3.scaleOrdinal(d3.schemeReds[8])
    ...
    const g = d3.select('#treemap').append('g')
...
function makeSimul() {
  ...
  const ln = [{ index: 0, source: ps[0], target: ps[1] }]

  const cs1 = d3.select('#simul').select('#c1')
  const cs2 = d3.select('#simul').select('#c2')
